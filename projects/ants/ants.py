...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            # BEGIN Problem 13
            if isinstance(insect, QueenAnt) and not insect.fake_queen:
                return
            "*** YOUR CODE HERE ***"
            # END Problem 13
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        return self.place.ant is not None and self.place.ant.block_paths
        # END Problem 7
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        places_checked = self.place
        check_next = 0
        while places_checked != hive and places_checked: #outside hive
            if check_next >= self.min_range and check_next <= self.max_range: #if within range
                #checks if place is occupied
                if_bee = random_or_none(places_checked.bees)
                if not if_bee:
                    places_checked = places_checked.entrance
                    check_next += 1
                else:
                    return if_bee
            else:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor - amount <= 0:
            for Bs in list(self.place.bees):
                Bs.reduce_armor(self.damage) #damage all Bs on place
        Insect.reduce_armor(self, amount) #check if anything is dead
        "*** YOUR CODE HERE ***"
        # END Problem 5
...
# The WallAnt class
class WallAnt(Ant):
    """WallAnt inherits from Ant, takes no action"""
    name = 'Wall'
    implemented = True
    food_cost = 4
    def __init__(self):
...
class BodyguardAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 9
            #This ant does not contain another ant.
            #The other ant is not a container.
        return self.is_container and not self.contained_ant and not other.is_container
        "*** YOUR CODE HERE ***"
        # END Problem 9
...
class TankAnt(BodyguardAnt):
    ...
    def action(self, colony):
        # BEGIN Problem 10
            #only difference between a TankAnt and a BodyguardAnt is damage
        BodyguardAnt.action(self, colony)
        for Bs in list(self.place.bees):
            Bs.reduce_armor(self.damage)
        "*** YOUR CODE HERE ***"
        # END Problem 10
...
# END Problem 13
    ...
    implemented = True   # Change to True to view in the GUI
    food_cost = 7
    fake_queen = False #imposter
    if_fake_queen = 0 #if >0 delete
    check_if_doubled = []
    # END Problem 13
    ...
    def __init__(self, armor=1):
        # BEGIN Problem 13
        Ant.__init__(self, armor)
        if QueenAnt.if_fake_queen >= 1:
            self.fake_queen = True
        QueenAnt.if_fake_queen += 1
        "*** YOUR CODE HERE ***"
        # END Problem 13
    ...
    def action(self, colony):
        ...
        """
        # The QueenAnt doubles the damage of all the ants behind her each
        #time she performs an action. Once an ant's damage has been doubled,
        #it is not doubled again for subsequent turns.

        #kill self if not True Queen
        #If not queen, double everyone else
        #check self.place.exit if ants then double if true

        # BEGIN Problem 13
        if QueenAnt.if_fake_queen > 1 and self.fake_queen == True:
        ...
        #colony.queen = QueenPlace(self.place, colony.queen)
            ...
            while behind != None:
                ant = behind.ant
                if ant != None:
                    if ant not in QueenAnt.check_if_doubled:
                        ant.damage *= 2
                        QueenAnt.check_if_doubled.append(ant)
                    #repeat but for contained_ant
                    #these arguments are not related... can check separately
                    if ant.is_container:
                        if ant.contained_ant != None:
                            if ant.contained_ant not in QueenAnt.check_if_doubled:
                                ant.contained_ant.damage *= 2
                                QueenAnt.check_if_doubled.append(ant.contained_ant)
                behind = behind.exit
        ScubaThrower.action(self, colony)

        #def damage_doubled(place):
        #    if not type(place.ant) == QueenAnt:
        #        if place.ant is not None and place.ant.is_doubled is not True:
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 13
        self.armor -= amount
        if not self.fake_queen and self.armor <= 0:
            self.place.remove_insect(self)
            bees_win()
        "*** YOUR CODE HERE ***"
        # END Problem 13
