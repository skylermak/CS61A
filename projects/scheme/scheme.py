...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 5
        first_expr = scheme_eval(first, env)
        if scheme_procedurep(first_expr):
            return scheme_apply(first_expr, rest.map(lambda x:scheme_eval(x,env)), env)
        else:
            raise SchemeError
        "*** YOUR CODE HERE ***"
        # END PROBLEM 5
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        x = LambdaProcedure(target.second, expressions.second, env)
        env.define(target.first, x)
        return target.first
        "*** YOUR CODE HERE ***"
        # END PROBLEM 10
    else:
...
def do_and_form(expressions, env):
    ...
    if expressions == nil:
        ...
        #check if first false (if false return false) (if true continue)
...
def do_or_form(expressions, env):
    ...
    if expressions == nil:
        ...
        #check if first is false (if false continue) (if true return it))
        x = scheme_eval(expressions.first, env)
        if scheme_falsep(x):
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 15
    #x = env.make_child_frame(nil, nil)
    #check_form(bindings, 2, 2)
    #check_formals(Pair(b.first, nil))
    #check_form(bindings, 2, 2)
    #for b in bindings:
    #    check_form(b, 2, 2)
    #    check_formals(Pair(b.first, nil))
    #    x.define(b.first, scheme_eval(b.second, env))
    #return x
    ...
    while bindings is not nil:
        b = bindings.first
        check_form(b, 2, 2)
        formals = Pair(b.first, formals)
        check_formals(formals)
        vals = Pair(scheme_eval(b.second.first, env), vals)
