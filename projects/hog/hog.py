...
def free_bacon(score):
    ...
    # BEGIN PROBLEM 2
    first_integer = score // 10
    second_integer = score % 10
    after_score = 2 * first_integer - second_integer
    if after_score > 1:
        return 2 * first_integer - second_integer
    else:
        return 1
    # END PROBLEM 2
...
def announce_highest(who, previous_high=0, previous_score=0):
    ...
    def highest_score(score0, score1):
        if who:
            ...
            score = score0
        new_high_score_check = score - previous_score
        new_high = previous_high
        if new_high < new_high_score_check:
            new_high = new_high_score_check
            print(new_high_score_check, "point(s)! That's the biggest gain yet for Player", who)
...
def make_averaged(fn, num_samples=1000):
    ...
    # BEGIN PROBLEM 8
    def average(*args):
        total, k = 0, num_samples
        while k:
            total += fn(*args)
            k -= 1
        average_statement = total / num_samples
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    good_roll = 0
    biggest_average = 0
    functionlol = make_averaged(roll_dice, 1000)
    while test_roll <= 10:
        x = functionlol(test_roll, dice)
        if x > biggest_average:
            biggest_average = x
            good_roll = test_roll
        test_roll += 1
    return good_roll
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    if is_swap(free_bacon(opponent_score) + score, opponent_score):
        if opponent_score - (free_bacon(opponent_score) + score) >= margin:
            return 0
